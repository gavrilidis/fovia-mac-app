...
interface ToolbarProps {
  ...
  onHelp: () => void;
  onSettings: () => void;
  // Regroup: re-run clustering with the current similarity threshold on
  // every stored face of the active folder. Used after the user bumps
  // the threshold to fix under-clustering (e.g. 8 persons found where
  // there should be 13).
  onRegroup: () => void;
  regrouping: boolean;
  // Search
    ...
    >
      ...
      <div className="flex h-11 items-center gap-3 overflow-hidden px-4">
        ...
        <div className="flex flex-shrink-0 items-center gap-0.5">
          ...
          {/* Regroup Faces — rebuild person clusters with the current
              similarity threshold. Neutral styling so it sits next to
              the other macOS-native icon buttons. */}
          <IconBtn
            onClick={onRegroup}
            title={t("toolbar_regroup")}
            active={regrouping}
          >
            <svg className={`h-[15px] w-[15px] ${regrouping ? "animate-spin" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
